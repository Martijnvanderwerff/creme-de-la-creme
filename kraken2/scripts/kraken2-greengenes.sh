...
barcodes = ( seq -w 01 24 )

for barcode in "${barcodes[@]}"
do
  : 
  mkdir ${workdir}kraken2/T0_T1/greengenes/barcode${barcode}
  kraken2 --db ${workdir}kraken2-db-greengenes --threads $SLURM_CPUS_PER_TASK --use-names \
  --report ${workdir}kraken2/T0_T1/greengenes/barcode${barcode}/barcode${barcode}.txt \
  ${datadir}barcode${barcode}*.fastq --output ${workdir}kraken2/T0_T1/greengenes/barcode${barcode}.kraken2
done
